...
function startQuiz(mode) {
    ...
    // Quick mode: shuffle then take 5. Final mode: shuffle the FULL question order too,
    // so repeat attempts (e.g. retrying for the 85% unlock) don't just memorize positions.
    let baseData = (mode === 'quick') ? shuffleArray(quizData).slice(0, 5) : shuffleArray(quizData);

    activeQuizData = baseData.map(q => ({
        ...q,
        // Resolve option ids -> {id, romaji, audio} and shuffle their order
        o: shuffleArray(q.options.map(id => ({ id, ...phrases[id] })))
    }));
...
function renderQuestion() {
    ...
    document.getElementById('quiz-content').innerHTML = `
        ...
        <div class="grid gap-3" id="answer-grid">
            ${q.o.map(opt => `
                <div class="flex gap-2">
                    <button data-answer-id="${opt.id}" onclick="handleAnswer('${opt.id}', this)" class="answer-btn flex-grow bg-white text-sky-600 p-3 rounded-xl hover:bg-sky-100 transition shadow-sm font-bold">${opt.romaji}</button>
                    <button onclick="playAudio('${opt.audio}')" aria-label="Play pronunciation for ${opt.romaji}" title="Play pronunciation" class="bg-white text-sky-600 px-3 py-2 rounded-xl hover:bg-sky-100 shadow-sm flex items-center">
...
function handleAnswer(selectedId, btnEl) {
    if (answerLocked) return; // ignore double taps while feedback is showing
    answerLocked = true;

    const q = activeQuizData[state.currentIdx];
    const isCorrect = selectedId === q.answer;

    // Disable all answer buttons and color-code them so the user can SEE
    // what was right/wrong before the next question loads.
    const allBtns = document.querySelectorAll('#answer-grid .answer-btn');
    allBtns.forEach(b => {
        b.disabled = true;
        b.classList.remove('hover:bg-sky-100');
        if (b.dataset.answerId === q.answer) {
    ...
    if (isCorrect) {
        ...
        updateKitsune('wrong');
        document.getElementById('kitsune-speech').innerHTML = `Not quite! The correct answer was: <b>${phrases[q.answer].romaji}</b>. ${q.tip || ''}`;
    }

    state.results.push({
        q: q.q,
        selected: phrases[selectedId].romaji,
        correct: isCorrect,
        correctAns: phrases[q.answer].romaji
    });
...
function checkUnlockStatus() {
    ...
    if (isUnlocked) {
        ...
        // Add star to Final Exam button. IMPORTANT: never use finalBtn.innerText += '...'
        // here - setting .innerText replaces ALL child nodes (including the SVG icon
        // and text span) with a single text node, silently destroying the icon. Instead
        // we just reveal a dedicated star element that already exists in the markup.
        const finalStar = document.getElementById('final-exam-star');
        if (finalStar) finalStar.classList.remove('hidden');
...
// Listen for user activity. Added 'scroll' and 'mousemove' so someone quietly
// reading (e.g. the quiz summary review list) doesn't get falsely flagged idle.
['mousedown', 'keydown', 'touchstart', 'scroll', 'mousemove'].forEach(evt =>
